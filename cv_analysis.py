...
class CVAnalyzer:
    ...
    def _get_outlier_cycles(self, cycle_results: List[Dict]) -> List[int]:
        """
        获取被排除离群值的轮次（基于z-score排除的轮次）
        ...
        Returns:
            被排除离群值的轮次列表
        """
        # 获取有效的电容值（排除异常值和容值为0的）
        valid_results = [r for r in cycle_results 
                        if not r.get('is_outlier', False) and r['capacitance'] > 0]
        
        if len(valid_results) <= self.outlier_count:
            return []
        
        capacitances = [r['capacitance'] for r in valid_results]
        
        # 按离群程度排序（使用z-score）
        mean = statistics.mean(capacitances)
        stdev = statistics.stdev(capacitances) if len(capacitances) > 1 else 0
        
        if stdev == 0:
            return []
        
        # 创建包含z-score和轮次的列表
        z_scores_with_cycles = []
        for result in valid_results:
            z_score = abs(result['capacitance'] - mean) / stdev
            z_scores_with_cycles.append((z_score, result['cycle_num']))
        
        # 按z-score降序排序
        z_scores_with_cycles.sort(reverse=True, key=lambda x: x[0])
        
        # 获取被排除的轮次（前outlier_count个）
        excluded_cycles = [cycle_num for _, cycle_num in z_scores_with_cycles[:self.outlier_count]]
        
        return sorted(excluded_cycles)
